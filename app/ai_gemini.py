...
# (connect timeout, read timeout) in seconds. Read timeout was 90s flat before, which is
# tight for a slow/variable connection to Google's API - split it out and give it more
# room, and retry transient timeouts/connection drops instead of failing on the first one.
...
def _post_with_retry(url, **kwargs):
    ...
    raise AIConfigError(
        f"Could not reach the Gemini API after {MAX_RETRIES + 1} attempts "
        f"(last error: {last_err}). Check your internet connection / firewall, or try again."
    )
